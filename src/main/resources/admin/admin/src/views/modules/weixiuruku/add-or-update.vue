<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="维修编号" prop="weixiubianhao">
          <el-select :disabled="ro.weixiubianhao" @change="weixiubianhaoChange" v-model="ruleForm.weixiubianhao" placeholder="请选择维修编号">
            <el-option
                v-for="(item,index) in weixiubianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="维修情况" prop="weixiuqingkuang">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.weixiuqingkuang" label="维修情况" prop="weixiuqingkuang">
                    <span v-html="ruleForm.weixiuqingkuang"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          weixiuqingkuang: [
                { required: true, message: '维修情况不能为空', trigger: 'blur' },
          ],
          beizhu: [
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.yuangongxingming!=''&&json.yuangongxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shebeibaoxiu/weixiubianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.weixiubianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    weixiubianhaoChange () {
      this.$http({
        url: `follow/shebeibaoxiu/weixiubianhao?columnValue=`+ this.ruleForm.weixiubianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.shebeixiangqing = this.ruleForm.shebeixiangqing.replace(reg,'../../../springboot1332i/upload');
	this.ruleForm.weixiuqingkuang = this.ruleForm.weixiuqingkuang.replace(reg,'../../../springboot1332i/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weixiuruku/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weixiurukuCrossAddOrUpdateFlag = false;
						 this.parent.search();

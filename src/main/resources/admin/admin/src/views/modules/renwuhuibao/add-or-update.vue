<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="要求时间" prop="yaoqiushijian">
          <el-input v-model="ruleForm.yaoqiushijian" 
              placeholder="要求时间" clearable  :readonly="ro.yaoqiushijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="要求时间" prop="yaoqiushijian">
              <el-input v-model="ruleForm.yaoqiushijian" 
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="汇报时间" prop="huibaoshijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huibaoshijian" label="汇报时间" prop="huibaoshijian">
              <el-input v-model="ruleForm.huibaoshijian" 
                placeholder="汇报时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="回收物" prop="huishouwu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.huishouwu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.huishouwu" label="回收物" prop="huishouwu">
                    <span v-html="ruleForm.huishouwu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongzuoneirong = true;
          ...
          if(o=='huishouwu'){
            this.ruleForm.huishouwu = obj[o];
	    this.ro.huishouwu = true;
            continue;
          }
          if(o=='lianxiren'){
	...
	this.ruleForm.huishouwu = this.ruleForm.huishouwu.replace(reg,'../../../springboot1332i/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `renwuhuibao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.renwuhuibaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
